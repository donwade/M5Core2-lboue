...
void *TFT_eSprite::callocSprite(int16_t w, int16_t h, uint8_t frames) {
    ...
    if (_bpp == 16) {
        if (psramFound() && _usePsram)
            ...
            // Since it's a M5Core2, disabled psram for this sprite **is** a
            // user choice, using heap_caps_calloc instead of calloc prevents
            // auto assignation to psram
            ptr8 = (uint8_t *)heap_caps_calloc(w * h + 1, sizeof(uint16_t),
                                               MALLOC_CAP_8BIT);
    } else if (_bpp == 8) {
        if (psramFound() && _usePsram)
            ...
            // user choice, using heap_caps_calloc instead of calloc prevents
            // auto assignation to psram
            ptr8 = (uint8_t *)heap_caps_calloc(w * h + 1, sizeof(uint8_t),
                                               MALLOC_CAP_8BIT);
    } else  // Must be 1 bpp
    {
        ...
        if (psramFound() && _usePsram)
            ...
            // auto assignation to psram
            ptr8 =
                (uint8_t *)heap_caps_calloc(frames * (w >> 3) * h + frames,
    ...
    // Font is not 2 and hence is RLE encoded
    {
        ...
        if (_bpp == 16)
            ...
            color = ((textcolor & 0xE000) >> 8 | (textcolor & 0x0700) >> 6 |
                     (textcolor & 0x0018) >> 3);
        int32_t px = 0,
                py = pY;  // To hold character block start and end column and
                          // row values
